...
export default async (req: Request, res: Response, next: NextFunction) => {
  ...
  res.locals.loginUser = req.user.dataValues;
  ...
  const newPagination = (totalCount: number) => {
    ...
    if (totalCount > 0) {
      ...
      html += `<li class="paginate_button page-item next${className}"><a class="page-link button-page first" href="${href}">最初</a></li>`;
      href = 'javascript:void(0)';
      if (offset !== 0) {
        query.offset = offset - pageLine;
        href = `${req.baseUrl}?${qs.stringify(query)}`;
      }
      html += `<li class="paginate_button page-item next${className}"><a class="page-link button-page laquo" href="${href}">前へ</a></li>`;
      ...
      html += `<li class="paginate_button${className} page-item"><a class="page-link button-page raquo" href="${href}">次へ</a></li>`;
      href = 'javascript:void(0)';
      if (offset + pageLine < totalCount) {
        query.offset = Math.floor((totalCount - 1) / pageLine) * pageLine;
        href = `${req.baseUrl}?${qs.stringify(query)}`;
      }
      html += `<li class="paginate_button page-item next${className}"><a class="page-link button-page last" href="${href}">最終</a></li>`;
      html += `</ul></div>`;
    } else {
      html = `
